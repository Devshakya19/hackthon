  ...
  const sidebar = (
    <aside className="hidden lg:flex fixed inset-y-0 left-0 w-72 flex-col border-r border-white/10 bg-bg/95 backdrop-blur-xl px-5 py-6">
      <div className="flex items-center gap-3 px-2 pb-6 border-b border-white/10">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary text-bg font-black">
          ADM
        </div>
        <div>
          <div className="text-lg font-bold text-text-900">Admin Panel</div>
          <div className="text-xs uppercase tracking-[0.28em] text-text-500">
            Platform Controls
          </div>
        </div>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink
              ...
              to={item.id}
              className={({ isActive }) =>
                `flex w-full items-center rounded-2xl px-4 py-3 text-left text-sm font-medium transition-colors ${
                  isActive
      ...
      <div className="mt-auto rounded-3xl border border-white/10 bg-white/5 p-4">
        ...
        <div className="mt-3 flex gap-2">
          <button
            ...
            className="rounded-2xl bg-primary px-4 py-2 text-sm font-semibold text-bg"
          >
            Sign out
          </button>
          <Link
            to="/"
            className="rounded-2xl border border-white/10 px-4 py-2 text-sm text-text-500"
          >
            Home
          </Link>
        </div>
  ...
  const topbar = (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-bg/85 backdrop-blur-xl">
      <div className="flex items-center gap-3 px-4 py-4 sm:px-6 lg:px-8">
        <button
          className="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 p-2 text-text-900 lg:hidden"
        ...
        <div className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
          <input
            ...
            className="w-full bg-transparent text-sm text-text-900 placeholder:text-text-500 focus:outline-none"
          />
        </div>
        ...
        <button
          onClick={handleSignOut}
          className="hidden sm:inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-bg"
        >
