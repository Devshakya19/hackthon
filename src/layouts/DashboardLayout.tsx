...
export default function DashboardLayout() {
	...
	const sidebar = el(
		'aside',
		{ className: 'hidden lg:flex fixed inset-y-0 left-0 w-72 flex-col border-r border-white/10 bg-bg/95 backdrop-blur-xl px-5 py-6' },
		el(
			'div',
			{ className: 'flex items-center gap-3 px-2 pb-6 border-b border-white/10' },
			el('div', { className: 'flex h-11 w-11 items-center justify-center rounded-2xl bg-primary text-bg font-black shadow-[0_0_24px_rgba(0,240,255,0.25)]' }, '32'),
			el(
				'div',
				null,
		...
		el(
			...
			el('div', { className: 'text-xs uppercase tracking-[0.3em] text-text-500' }, 'Active Role'),
			el(
				'div',
	...
	const topbar = el(
		...
		el(
			...
			el('button', { className: 'inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 p-2 text-text-900 lg:hidden', onClick: () => setMobileOpen(true), 'aria-label': 'Open navigation' }, el(Menu, { className: 'h-5 w-5' })),
			el(
				'div',
			...
			el(
				'div',
				{ className: 'hidden md:flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3' },
				el('div', { className: 'h-9 w-9 rounded-xl bg-primary/15 text-center text-sm font-bold leading-9 text-primary' }, 'AL'),
				el(
					'div',
					null,
					el('div', { className: 'text-sm font-semibold text-text-900' }, displayName),
					el('div', { className: 'text-xs text-text-500' }, profile?.team_name || 'Team Alpha'),
				),
			),
			el('button', { onClick: handleSignOut, className: 'hidden sm:inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-bg shadow-[0_0_18px_rgba(0,240,255,0.22)]' }, el(LogOut, { className: 'h-4 w-4' }), 'Sign out'),
		),
	)
	...
	const mobileDrawer = mobileOpen
		? el(
			...
			el(
				'aside',
				{ className: 'flex h-full w-80 flex-col border-r border-white/10 bg-bg px-5 py-6' },
				el(
					'div',
					{ className: 'flex items-center justify-between border-b border-white/10 pb-5' },
					el('div', { className: 'text-lg font-bold text-text-900' }, 'Navigation'),
					el('button', { className: 'rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-text-900', onClick: () => setMobileOpen(false) }, 'Close'),
				),
