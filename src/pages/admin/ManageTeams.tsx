...
export default function ManageTeams() {
  ...
  useEffect(() => {
    async function load() {
      const [teamsResult, membersResult, roomsResult, problemsResult] =
        await Promise.all([
          listTeams(),
          listAllTeamMembers(),
          listRooms(),
  ...
  const membersByTeam = useMemo(() => {
    return members.reduce<Record<string, TeamMemberRow[]>>(
      (accumulator, member) => {
    ...
    <div className="space-y-6">
      <div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6">
        ...
        <h1 className="mt-2 text-2xl font-bold text-text-900">
          Inspect teams, members, and assignments
        </h1>
      </div>
          ...
          >
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-3">
                ...
                <div className="text-sm text-text-500">
                  Team UID: {team.team_uid}
                </div>
                <div className="text-sm text-text-500">
                  Problem: {team.problem_id || "--"}
                </div>
                <div className="text-sm text-text-500">
                  Room: {team.room_id || "--"}
                </div>
                <div className="text-sm text-text-500">
                  Seat: {team.seat_number || "--"}
                </div>
              ...
              <div className="grid gap-3 md:grid-cols-3 lg:w-[42rem]">
                <select
                  value={team.room_id ?? ""}
                  onChange={(e) =>
                    saveTeam(team.id, { room_id: e.target.value || null })
                  }
                  className="rounded-2xl border border-white/10 bg-bg/70 px-4 py-3 text-sm text-text-900 outline-none"
                >
                ...
                <input
                  value={team.seat_number ?? ""}
                  onChange={(e) =>
                    saveTeam(team.id, { seat_number: e.target.value || null })
                  }
                  placeholder="Seat number"
                  className="rounded-2xl border border-white/10 bg-bg/70 px-4 py-3 text-sm text-text-900 outline-none"
                />
                <select
                  value={team.problem_id ?? ""}
                  onChange={(e) =>
                    saveTeam(team.id, { problem_id: e.target.value || null })
                  }
                  className="rounded-2xl border border-white/10 bg-bg/70 px-4 py-3 text-sm text-text-900 outline-none"
                >
