...
export default function ManageRooms() {
  ...
  async function saveRoom() {
    if (!form.block.trim() || !form.room_number.trim())
      return setMessage("Block and room number are required.");
    if (editingId) {
      const { error } = await updateRoom(editingId, {
      ...
      setRooms((current) =>
        current.map((room) =>
          room.id === editingId
            ? { ...room, ...form, capacity: Number(form.capacity) || 0 }
            : room
        )
      );
      ...
      <div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6">
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-[1fr_1fr_160px_auto]">
          <input
            value={form.block}
            onChange={(e) =>
              setForm((current) => ({ ...current, block: e.target.value }))
            }
            placeholder="Block"
