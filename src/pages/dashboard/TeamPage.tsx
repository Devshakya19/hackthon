...
import { getTeamById, type TeamMemberRow, type TeamRow } from '../../supabase/database'
import { addTeamMember, getTeamByLeaderId, getTeamMembers, removeTeamMember, updateTeam, updateTeamMember } from '../../supabase/queries'
...
export default function TeamPage() {
	...
	const [newMember, setNewMember] = useState<MemberForm>({ name: '', email: '' })
	const [editingId, setEditingId] = useState<string | null>(null)
	const [editingMember, setEditingMember] = useState<MemberForm>({ name: '', email: '' })
	...
	async function saveTeamName() {
		...
		const { error } = await updateTeam(team.id, { team_name: teamName.trim() || team.team_name })
		if (error) return setMessage(error.message)
		setTeam((current) => (current ? { ...current, team_name: teamName.trim() || current.team_name } : current))
	...
	async function addMember() {
		...
		const row: TeamMemberRow = { id: crypto.randomUUID(), team_id: team.id, name: newMember.name.trim(), email: newMember.email.trim() || 'member@amity.edu' }
	...
	async function saveMember(memberId: string) {
		const { error } = await updateTeamMember(memberId, { name: editingMember.name.trim(), email: editingMember.email.trim() })
		if (error) return setMessage(error.message)
		setMembers((current) => current.map((member) => (member.id === memberId ? { ...member, ...editingMember } : member)))
		setEditingId(null)
	...
	if (loading) {
		return <div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6 text-sm text-text-500">Loading team...</div>
	}
	...
	return (
		<div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6">
			...
			{role !== 'member' ? (
				<>
					<div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto]">
						...
						<button onClick={saveTeamName} className="rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-bg">Save team name</button>
					</div>

					<div className="mt-6 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
						<input value={newMember.name} onChange={(e) => setNewMember((current) => ({ ...current, name: e.target.value }))} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-text-900 outline-none" placeholder="Member name" />
						<input value={newMember.email} onChange={(e) => setNewMember((current) => ({ ...current, email: e.target.value }))} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-text-900 outline-none" placeholder="Member email" />
						<button onClick={addMember} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-bg"><Plus className="h-4 w-4" />Add</button>
					</div>
				</>
			) : null}
			...
			{message ? <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-text-500">{message}</div> : null}
		</div>
	)
}
