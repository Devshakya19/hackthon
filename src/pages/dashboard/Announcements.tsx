...
export default function Announcements() {
		...
		async function loadAnnouncements() {
			...
			if (!profile?.team_id) {
				try {
					...
					if (!active) return
					setMessage(error instanceof Error ? error.message : 'Unable to load team context')
				}
			}
	...
	if (loading) {
		return <div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6 text-sm text-text-500">Loading announcements...</div>
	}
	...
	return (
		<div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6">
			...
			<h1 className="mt-2 text-2xl font-bold text-text-900">Latest updates from the organizers</h1>
