		...
		async function loadHome() {
			...
			if (!teamResult.data) {
				try {
					...
					if (!active) return
					setMessage(error instanceof Error ? error.message : 'Unable to load team record')
				}
			}
	...
	return (
		<div className="space-y-6">
			<section className="grid gap-4 lg:grid-cols-4">
				<SummaryCard label="Team Name" value={team?.team_name || profile?.team_name || 'Team'} hint="Loaded from Supabase" />
				<SummaryCard label="Team Members" value={String(members.length)} hint="Names only in overview" />
				<SummaryCard label="Seat" value={seatValue} hint="Not editable from overview" />
				<SummaryCard label="Problem" value={selectedProblem?.title || 'Not selected'} hint={selectedProblem ? 'Stored in teams.problem_id' : 'No problem selected yet'} />
			</section>

			<section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
				<div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6">
					<div className="text-sm uppercase tracking-[0.3em] text-text-500">Team Members</div>
					<h2 className="mt-2 text-2xl font-bold text-text-900">Current roster</h2>
					<div className="mt-4 grid gap-3 md:grid-cols-2">
						...
						)) : <div className="text-sm text-text-500">No team members added yet.</div>}
					</div>
				</div>

				<div className="space-y-6">
					<div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6">
						<div className="text-sm uppercase tracking-[0.3em] text-text-500">Quick Status</div>
					...
					<div className="glass-card rounded-3xl border-white/10 bg-white/5 p-6">
						<div className="text-sm uppercase tracking-[0.3em] text-text-500">Status</div>
						<div className="mt-3 text-sm text-text-500">{message || 'Use the sidebar to open Team, Problems, Announcements, or Submission pages.'}</div>
					</div>
				</div>
			</section>
		</div>
