...
export default function VerifyEmail() {
  ...
  React.useEffect(() => {
    ...
    const savedAt = Number(window.localStorage.getItem(cooldownKey(initialEmail)) ?? '0')
    if (savedAt) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const savedAt = Number(window.localStorage.getItem(cooldownKey(trimmedEmail)) ?? '0')
    const remaining = Math.max(0, RESEND_COOLDOWN_MS - (Date.now() - savedAt))

    if (remaining > 0) {
      setError(`Please wait ${Math.ceil(remaining / 1000)} seconds before requesting another link.`)
      setCooldownLeft(Math.ceil(remaining / 1000))
      return
    }
  ...
  return (
    <div className="min-h-screen text-text-900 bg-bg font-sans flex items-center justify-center relative overflow-hidden px-6">
      ...
      >
        {/* Back Link */}
        <Link to="/register" className="inline-flex items-center gap-2 text-primary hover:text-white transition-colors mb-6 text-sm font-semibold">
        ...
        <div className="glass-card border border-white/10 p-8 md:p-10 shadow-2xl relative overflow-hidden group">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-text-500 text-sm mt-2">Supabase sends a confirmation link to your inbox.</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-xs uppercase tracking-widest text-text-500 font-semibold mb-2">Email Address</label>
              <input
                type="email"
                required
                value={email}
            ...
            >
              {loading ? (
                <div className="w-5 h-5 border-2 border-bg border-t-transparent rounded-full animate-spin" />
                  ...
                  cooldownLeft > 0 ? `Wait ${cooldownLeft}s` : 'Resend Verification Link'
              )}
            </button>
