...
export default function ForgotPassword() {
  ...
  React.useEffect(() => {
    const savedAt = Number(
      window.localStorage.getItem(resetCooldownKey(email)) ?? "0"
    );
    if (savedAt) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const savedAt = Number(
      window.localStorage.getItem(resetCooldownKey(trimmedEmail)) ?? "0"
    );
    const remaining = Math.max(0, RESET_COOLDOWN_MS - (Date.now() - savedAt));
    if (remaining > 0) {
      setError(
          ...
          {!submitted ? (
            ...
            >
              ...
              <p className="text-text-500 text-sm leading-relaxed mb-6">
                If the email exists in our directory, a reset token has been
                dispatched. Please inspect your inbox.
              </p>
              <Link
                to="/login"
