...
const prizes = [
  {
    title: "Winner Prize",
    desc: "Main trophy + ₹50,000 + internships",
    icon: "🏆",
  },
  { title: "Runner-Up", desc: "₹30,000 + swag kits", icon: "🥈" },
  {
    title: "Cybersecurity Excellence",
    desc: "Pro Security tool subscriptions",
    icon: "🛡️",
  },
  {
    title: "Innovation Award",
    desc: "Mentorship + product credits",
    icon: "💡",
  },
];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {prizes.map((p, idx) => (
          <motion.div
            ...
            className="glass-card group text-center relative overflow-hidden border border-white/10 hover:border-primary/50 transition-all duration-300 transform hover:-translate-y-2 hover:shadow-[0_10px_30px_rgba(191,0,255,0.2)]"
          >
            <div className="text-5xl mb-6 transform group-hover:scale-110 transition-transform duration-300">
              {p.icon}
            </div>
            <div className="text-2xl font-bold mb-3 text-white group-hover:text-primary transition-colors">
              {p.title}
            </div>
